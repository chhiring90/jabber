...
const Button = ({ children, buttonType, type, disabled, customClass, clicked }) => {
    ...
    switch (buttonType) {
        case 'anchor':
            ...
            break;
        case 'button':
            buttonElement = <button onClick={clicked} disabled={disabled} className={classes} type={type}>{children}</button>;
            break;
        default:
            buttonElement = <button onClick={clicked} disabled={disabled} className={customClass} type={type}>{children}</button>;
            break;
    }
