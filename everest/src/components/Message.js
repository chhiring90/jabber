...
const message = ({ userCurrent, messageBody }) => {
    ...
    if (userCurrent === 'jabber-admin') {
        messageBox = (
            <div className="flex text-xs text-blue-400 font-semibold text-center items-center mb-2">
                <span className="flex flex-1 w-full h-0.5 bg-brand-tertiary opacity-5 rounded-full"></span>
                <span className="flex-shrink px-2">{messageBody}</span>
                <span className="flex flex-1 w-full h-0.5 bg-brand-tertiary opacity-5 rounded-full"></span>
            </div>)
    }
